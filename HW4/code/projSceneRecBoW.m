...
n = length(train_image_paths);
m = length(test_image_paths);
f=[];

% for i=1:n
%     image = im2double(imread(train_image_paths{i}));
%     f(i,:) = size(image);
% end
% for i=1:m
%     image = im2double(imread(test_image_paths{i}));
%     f(i+n,:) = size(image);
% end
% disp(f);
%  image = im2double(imread(train_image_paths{1}));
%   features = extractHOGFeatures(image(1:16,1:16), 'CellSize',[4 4],'BlockSize',[1 1]);
% % % %     
% %  disp(features);
%   disp(size(features));
% disp(size(image));
...
% %plot(visualization);
% 
% vocab = build_vocabulary(train_image_paths, 200);
% disp(size(vocab));
...
switch lower(FEATURE)    
    ...
    case 'bag of words'
        ...
        if ~exist('train_image_features.mat', 'file')
            train_image_feats = get_bags_of_words(train_image_paths);
            save('train_image_features.mat', 'train_image_feats')
        else
            load('train_image_features.mat')
        end
        % Possibly write out train_image_features here as a *.mat
        if ~exist('test_image_features.mat', 'file')
            test_image_feats  = get_bags_of_words(test_image_paths);
            save('test_image_features.mat', 'test_image_feats')
        else
            load('test_image_features.mat')
        end
